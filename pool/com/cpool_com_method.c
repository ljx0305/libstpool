...
static inline cpool_core_t *CORE(cpool_ctx_t ctx)
{
	/**
	 * The first member of the context must be the address of the core object
	 */
	assert (sizeof(void *) == sizeof(long));
	return (cpool_core_t *)*(long *)ctx;
}

void  
cpool_com_atexit(cpool_ctx_t ctx, void (*__atexit)(void *), void *opaque)
{
	cpool_core_atexit(CORE(ctx), __atexit, opaque);
}

void
cpool_com_adjust(cpool_ctx_t ctx, int max, int min)
{
	cpool_core_t *core = CORE(ctx);
...
void
cpool_com_adjust_abs(cpool_ctx_t ctx, int max, int min)
{
	cpool_core_t *core = CORE(ctx);
...
int   
cpool_com_flush(cpool_ctx_t ctx)
{
	int ndecs;
	cpool_core_t *core = CORE(ctx);
...
void  
cpool_com_resume(cpool_ctx_t ctx)
{	
	cpool_core_resume(CORE(ctx));
}

long  
cpool_com_addref(cpool_ctx_t ctx)
{
	long ref = 0;
	cpool_core_t *core = CORE(ctx);
...
long  
cpool_com_release(cpool_ctx_t ctx)
{
	/**
	 * We do not waste our time on waiting for 
	 * the pool's being destroyed completely 
	 */
	return cpool_core_release_ex(CORE(ctx), 0, 0);
}

void  
cpool_com_setattr(cpool_ctx_t ctx, struct thread_attr *attr)
{
	cpool_core_t *core = CORE(ctx);
...
void  
cpool_com_getattr(cpool_ctx_t ctx, struct thread_attr *attr)
{
	cpool_core_t *core = CORE(ctx);
...
void  
cpool_com_set_activetimeo(cpool_ctx_t ctx, long acttimeo, long randtimeo)
{
	cpool_core_t *core = CORE(ctx);
...
ctask_t *
cpool_com_cache_get(cpool_ctx_t ctx)
{
	ctask_t *ptask;
	cpool_core_t *core = CORE(ctx);
...
void  
cpool_com_cache_put(cpool_ctx_t ctx, ctask_t *ptask)
{
	cpool_core_t *core = CORE(ctx);
