...
struct ___dummy {
	...
	OSPX_em_t *em;
#ifdef CONFIG_OPSX_ERR_THREAD_SAFE
	OSPX_pthread_rwlock_t lock;
#endif
} ospx_ectx;

#ifdef CONFIG_OSPX_ERR_THREAD_SAFE
...
#define RWLOCK_READER_ACQUIRE(ctx)  
#define RWLOCK_WRITER_ACQUIRE(ctx)  
#define RWLOCK_RELEASE(ctx)         
#endif
...
EXPORT OSPX_ansi_error 
OSPX_efunc(uint8_t m) 
{
...
EXPORT void 
OSPX_error_unregister(uint8_t m) 
{	
	RWLOCK_WRITER_ACQUIRE(&ospx_ectx);
	if (!OSPX_m_isfree(m)) {
		OSPX_em_t *ctx;
...
EXPORT void 
OSPX_setlasterror(OSPX_error_t error) 
{
	OSPX_tls_t *tls;
...
EXPORT const char *
OSPX_strerror(OSPX_error_t error) 
{
	int handled = 0;
...
EXPORT const char *
OSPX_sys_strerror(uint32_t code) 
{
#ifdef _WIN
	static struct {
		uint32_t error;
...
EXPORT void 
OSPX_errprefix_append(const char *fmt, ...) 
{
	OSPX_tls_t *tls;
...
EXPORT void 
OSPX_setlasterror2(OSPX_error_t error)
{
	OSPX_tls_t *tls;
